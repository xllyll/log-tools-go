...
// ImportMultiVolumeArchiveUnlocked 在已持有 device 解压锁时调用。
func (s *StorageService) ImportMultiVolumeArchiveUnlocked(ctx context.Context, deviceID, displayName string, partPaths []string) ([]string, error) {
	if len(partPaths) == 0 {
		return nil, fmt.Errorf("no volume parts")
	}
	dir := filepath.Dir(partPaths[0])
...
// PrepareVolumeDir creates a temp directory for volume parts under the device upload dir.
func (s *StorageService) PrepareVolumeDir(deviceID string) (string, error) {
	deviceDir, err := s.DeviceUploadDir(deviceID)
	if err != nil {
		return "", err
	}
	return os.MkdirTemp(deviceDir, "upload-vol-*")
}
